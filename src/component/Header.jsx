...
const location = useLocation();
  ...
  // keep userId in sync if other tabs log in/out
  useEffect(() => {
    const onStorage = (e) => {
      if (e.key === LOCAL_USER_ID_KEY) {
  ...
  {navItems.map((item) => (
    ...
    >
      {item.label}
                ...
                {/* (Optional) Cart icon – wire this to /cart if needed */}
                <button
                ...
                {/* Optional logout shortcut (show only if logged in) */}
                {isLoggedIn && (
                  <button
          ...
          <LoginDrawer
            ...
            onClose={() => setLoginOpen(false)}
            onSuccess={(user) => {
              // Expecting user object with _id
              try {
                if (user?._id) {
                  localStorage.setItem(LOCAL_USER_ID_KEY, user._id);
                  // store profile too if you have it:
                  if (user?.profile) {
                ...
                {["New Products", "Best Seller", "Register Warranty", "Loyalty"].map((page) => (
                  <a
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      setSelectedPage(page);
                      if (page === "Register Warranty") {
                        // If you want this to be gated, send to my-account (warranty tab) when logged in,
                        // otherwise open login drawer.
                        if (isLoggedIn) {
                          navigate("/my-account", { state: { tab: "warranty" } });
                        } else {
                          setLoginOpen(true);
                        }
                      }
