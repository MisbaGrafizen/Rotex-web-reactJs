...
//             />
//           ) : (
//             <div className="md77:w-[380px] rounded-xl bg-gray-100 h-[200px]" />
//           )}
//           {bottomC ? (
//             <BannerCard
//               src={bottomC.src}
...
function BannerCard({ src, alt, className = "" }) {
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-md md11:rounded-xl shadow-sm bg-[#f7f7f7] ${className}`}
      style={ratio ? { aspectRatio: ratio } : undefined}
    >
      <img
        src={src}
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (mounted) setBanners(normalized.length ? normalized : null);
      } catch {
        setBanners(null);
      }
    })();
  ...
  return (
    <section className="w-[90%] mx-auto">
      <div className="mx-auto md11:px-4">
        ...
        {/* Top row - responsive to stack on small screens */}
        <div className="flex flex-row  overflow-x-auto md77:flex-row gap-4 mb-4">
          {topLeft ? (
            <BannerCard src={topLeft.src} alt={topLeft.alt} className=" w-[330px] md77:w-full flex-shrink-0 lg:w-[68%] h-[300px]" />
          ) : (
            <div className="w-full lg:w-[68%] rounded-xl bg-gray-100 h-[300px]" />
          )}

          {topRight ? (
            <BannerCard src={topRight.src} alt={topRight.alt} className="  w-[330px] flex-shrink-0 md77:w-full lg:w-[32%] h-[300px]" />
          ) : (
            <div className="w-full lg:w-[32%] rounded-xl bg-gray-100 h-[300px]" />
          )}
        </div>

    <div className="md77:hidden block flex-col md:flex-row gap-4">
          {[ bottomA].map((item, i) =>
            item ? (
              <BannerCard
                key={i}
        ...
        {/* Bottom row - stacks into one column on small screens */}
        <div className="md77:flex hidden flex-col md:flex-row gap-4">
          {[bottomA, bottomB, bottomC].map((item, i) =>
            item ? (
              <BannerCard
                key={i}
                src={item.src}
