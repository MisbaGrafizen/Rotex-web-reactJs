...
import { Tag, CreditCard, Star, Check, X } from "lucide-react"
...
/** Normalize server coupon (handles {$oid} category ids) */
const normalizeCoupon = (c) => {
  const toId = (v) => {
    if (!v) return null
    if (typeof v === "string") return v
    if (typeof v === "object") return v._id || v.$oid || v.id || null
    return null
  }
  const type =
    String(c?.amountType || c?.type || "").toLowerCase() === "percentage"
      ? "percentage"
      : "flat"

  return {
    id: c?._id || c?.id || c?.couponCode,
    code: c?.couponCode || c?.code,
    title: c?.name || c?.title || c?.couponCode || "Coupon",
    description: c?.description || "",
    type,
    discount: Number(c?.amount || c?.discount || 0),
    minOrder: Number(c?.minOrder || 0),
    startDate: c?.startDate ? new Date(c.startDate) : null,
    endDate: c?.endDate ? new Date(c.endDate) : null,
    image: c?.image || "",
    categories: Array.isArray(c?.categories)
      ? c.categories.map(toId).filter(Boolean).map(String)
      : [],
    color: "from-blue-500 to-blue-600", // for your Tag bubble style
    _raw: c,
  }
}
...
export default function CheckoutPage() {
  ...
  const [isProcessing, setIsProcessing] = useState(false)
  const [paymentMethod, setPaymentMethod] = useState("upi")
  const [userDismissedAuto, setUserDismissedAuto] = useState(false);
  const location = useLocation()
  const shipping = 100
  ...
  useEffect(() => {
    if (couponFromState) setAppliedCoupon(couponFromState)
  }, [couponFromState])
  ...
  /** ---------- coupons: fetch + filter by product + date ---------- */
  const [coupons, setCoupons] = useState([])
  const [loadingCoupons, setLoadingCoupons] = useState(false)

  // single-product checkout → use that product id for category matching
  const productIdForCoupon = useMemo(() => {
    const pid = orderItems?.[0]?.id || lineItemFromState?.productId || null
    return pid ? String(pid) : null
  }, [orderItems, lineItemFromState])
  ...
  const displayCoupons = useMemo(() => {
    if (!Array.isArray(coupons) || coupons.length === 0 || !productIdForCoupon) return []
    const now = new Date()
    return coupons
      .filter((c) => {
        const dateOk = (!c.startDate || now >= c.startDate) && (!c.endDate || now <= c.endDate)
        const matches = c.categories.includes(String(productIdForCoupon))
        return dateOk && matches
      })
      .sort((a, b) => {
  ...
  const handleApplyListedCoupon = async (coupon) => {
    try {
      if (!productIdForCoupon) {
        setCouponError("No product to apply this coupon on.")
        return
      }
      ...
      if (!result?.valid) {
        setCouponError(result?.message || "Coupon not valid for this product.")
        return
      }
      const normalized = normalizeCoupon(result.coupon || coupon)
      setAppliedCoupon(normalized)
      setCouponError("")
    } catch (e) {
  ...
  const applyCoupon = async (e) => {
    ...
    try {
      ...
      const res = await ApiPost(APPLY_COUPON_URL, {
        couponCode: couponCode.trim(),
        categoryId: productIdForCoupon,
      })
      const json = res?.data
      if (!json?.success) {
      ...
      const normalized = normalizeCoupon(json.coupon)
      const sub = calculateSubtotal()
      if (normalized.minOrder && sub < normalized.minOrder) {
        setCouponError(`Minimum order value ₹${normalized.minOrder} required for ${normalized.code}`)
        return
      }
      setAppliedCoupon(normalized)
      setCouponError("")
  ...
  const handleRazorpayPayment = async () => {
    ...
    try {
      const amount = calculateTotal()
      const orderRes = await ApiPost(CREATE_RZP_ORDER_URL, { amount })
        ...
        handler: async (response) => {
          const payload = {
            ...
            orderItems: orderItems.map((it) => ({
              ...
              price: it.price,
              selectedColor: it.colorName,
              selectedColorImage: it.image,
            })),
            totalPrice: calculateSubtotal(),
          ...
          try {
            ...
            if (verifyRes?.status === 200 && (result?.success || result?._id)) {
              alert("Order placed successfully!")
            } else {
              alert(result?.message || "Failed to verify payment")
            }
  ...
  const createOrder = async () => {
    ...
    try {
      ...
      const finalAmount = subtotalVal + shipping - discountAmt
      ...
      const res = await ApiPost(CREATE_ORDER_URL, payload)
      const data = res?.data
          ...
          <div className="flex  w-[100%] gap-5">
            ...
            <div className=" w-[48%]">
              <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border border-blue-100 p-5 sticky top-8 hover:shadow-2xl transition-all duration-500">
                ...
                {/* ======= COUPONS (your original card design) ======= */}
                <div className="mb-4 p-4 bg-gradient-to-br from-blue-50 to-slate-50 rounded-xl border border-blue-100">
                  <div className="flex items-center mb-3">
                    <Tag className="w-6 h-6 mr-3" style={{ color: "#025da8" }} />
                    <span className="font-[600] text-slate-900">Available Coupons</span>
                  ...
                  {appliedCoupon ? (
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {loadingCoupons && (
                          <div className="text-sm text-gray-500 col-span-2">Loading coupons…</div>
                        )}

                        {!loadingCoupons && displayCoupons.length === 0 && (
                            ...
                            <div className="pr-10">
                              ...
                              {calculateSubtotal() < (coupon.minOrder || 0) && (
                                <div className="text-xs text-red-500 mt-1">
                                  Min. order: ₹{(coupon.minOrder || 0).toLocaleString("en-IN")}
                                </div>
                              )}
                            </div>
                          </div>
                          ...
                          <button
                            ...
                            style={{
                              background: !couponCode.trim()
                                ? "#9ca3af"
                                : "linear-gradient(135deg, #025da8, #0369a1)",
                              ...(couponCode.trim() && {
                                boxShadow: "0 4px 15px rgba(2, 93, 168, 0.3)",
                              }),
                            }}
                          >
                            Apply
